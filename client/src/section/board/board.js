...
const Board = ({ isLoggedIn }) => {
  ...
  useEffect(() => {
    ...
    if (havePostAsHost(privateDummy.host)) {
      setHasHost(true)
      setMyPost(privateDummy.host)
    } else {
      setHasHost(false)
      setMyPost(privateDummy.guest)
    }
      ...
      <div id="board-wrapper">
        ...
        <div id="board-list">
          <Routes>
            ...
            />
            <Route path="/private/*" element={<PrivateList hasHost={hasHost} />}>
              <Route
                path=""
                element={
                  hasHost ? (
                    <Post thisPost={myPost} confirmUsers={confirmUsers} />
                  ) : (
                    <PublicList list={wishListDummy} isWish={true} />
                  )
                }
              />
              <Route
